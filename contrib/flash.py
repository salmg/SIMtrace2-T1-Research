...
DEVICE_SIMTRACE = Device(usb_vendor_id=0x1d50, usb_product_id=0x60e3, name="SIMtrace 2", url={"trace": "https://ftp.osmocom.org/binaries/simtrace2/firmware/latest/simtrace-trace-dfu-latest.bin", "cardem": "https://ftp.osmocom.org/binaries/simtrace2/firmware/latest/simtrace-cardem-dfu-latest.bin"})
...
for usb_device in usb_devices:
	...
	dl_file.close()
	# compare versions
	dl_version = re.search(rb'firmware \d+\.\d+\.\d+\.\d+-[0-9a-fA-F]{4}', dl_data)
	...
	try:
		...
		# A firmware built from this tree reports "UNKNOWN": git-version-gen
		# needs a version tag with commits after it, and this fork has none, so
		# the Makefile bakes in UNKNOWN.
		#
		# An earlier revision treated that as "older than the download" and
		# flashed the Osmocom image. That is exactly backwards. Upstream cardem
		# has no T=1 block layer (firmware/libcommon/source/card_emu_t1.c is
		# ours), so overwriting a locally built image with the download silently
		# removes T=1 support from the board, and nothing in the output says so.
		#
		# An unidentifiable firmware is far more likely to be our own build than
		# something stale, so refuse and make the operator decide.
		print("device reports an unparseable version (" + version + ").")
		print("That is what a firmware built from this tree reports.")
		print("")
		print("Refusing to overwrite it with the Osmocom download: upstream")
		print("cardem has no T=1 block layer, so this would silently remove")
		print("T=1 support from the board.")
		print("")
		print("  to flash THIS tree's build (what you almost certainly want):")
		print("    make -C firmware BOARD=simtrace APP=cardem")
		print("    dfu-util --device 0x1d50:0x60e3 --path <path> --cfg 1 --alt 1 \\")
		print("             --reset --download firmware/bin/simtrace-cardem-dfu.bin")
		print("")
		print("  to flash the upstream image anyway, deliberately:")
		print("    " + sys.argv[0] + " --force-upstream ...")
		if "--force-upstream" not in sys.argv:
			os.remove(dl_path)
			continue
		print("--force-upstream given; flashing the Osmocom image.")
		versions = None
	dl_newer = True if versions is None else (versions[0] < dl_versions[0] or (versions[0] == dl_versions[0] and versions[1] < dl_versions[1]) or (versions[0] == dl_versions[0] and versions[1] == dl_versions[1] and versions[2] < dl_versions[2]) or (versions[0] == dl_versions[0] and versions[1] == dl_versions[1] and versions[2] == dl_versions[2] and versions[3] < dl_versions[3]))
	if not dl_newer:
...
if 0 == devices_nb:
	print("nothing to flash. Is the board plugged in, and -- in a virtual")
	print("machine -- passed through to the guest? A board that re-enumerates")
	print("is often dropped by the hypervisor and has to be re-attached.")
	print("Check with: lsusb | grep 1d50")
