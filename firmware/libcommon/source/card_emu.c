...
/* bit-mask of ISO 7816-3 protocols (1 << T) this card emulation can speak:
 * T=0 via the TPDU state machine below, T=1 via card_emu_t1.c */
#define SUPPORTED_PROTOCOLS	((1 << 0) | (1 << 1))
...
enum iso7816_3_card_state {
	...
	ISO_S_IN_T1,		/* T=1 block exchange */
};
...
struct card_handle {
	...
	/* transmission protocol currently in force (0 = T=0, 1 = T=1) */
	uint8_t proto;
	/* bit-mask of protocols the current ATR offers */
	uint32_t atr_proto_mask;
	/* error detection code the ATR selected for T=1 */
	enum t1_edc t1_edc;
	/* T=1 block layer, only meaningful while proto == 1 */
	struct t1_state t1;
	/* T=1: the reader is waiting on a block from us, so the next chunk the
	 * host hands over may go straight out. False while the reader still owes
	 * us the R-block that acknowledges the chunk already sent. */
	bool t1_owe_reader;
...
bool card_emu_ch_ready(const struct card_handle *ch)
{
	...
	case ISO_S_IN_TPDU:
	/* T=1 spends the whole data phase in one state, and the host's response
	 * arrives while we sit in it. Leaving it out here makes
	 * dispatch_usb_command_cardem() drop every response as "channel not
	 * ready", so the card falls silent and the reader times out and resets. */
	case ISO_S_IN_T1:
		return true;
	default:
		return false;
...
static void card_handle_reset(struct card_handle *ch)
{
	...
	/* a reset re-runs the ATR, which settles the protocol afresh; drop any
	 * half-received or half-sent T=1 block along with it */
	ch->proto = 0;
	ch->t1_owe_reader = false;
	t1_init(&ch->t1, ch->t1_edc, T1_DEFAULT_IFS);
}
			   ...
			   enum iso7816_3_card_state new_state)
{
	...
	case ISO_S_WAIT_TPDU:
		...
		break;
	case ISO_S_IN_T1:
		/* the block layer decides the direction from here on; start
		 * out listening for the reader's first block */
		card_emu_uart_enable(ch->uart_chan, ENABLE_RX);
		break;
	case ISO_S_IN_PTS:
	case ISO_S_IN_TPDU:
...
/*! Walk the ATR's interface bytes to see which protocols it offers.
 *  \param[out] mask one bit per offered protocol, i.e. (1 << T)
 *  \param[out] edc error detection code the ATR selects for T=1
 *  \returns the first protocol offered, which is the one in force until a
 *           PPS exchange says otherwise
 *
 *  ISO 7816-3 8.2: the low nibble of each TDi names a protocol and its high
 *  nibble says which of TA/TB/TC/TD follow it. An ATR carrying no TDi at all
 *  offers T=0 only. Per 11.4.2 the first TC following a TDi that names T=1
 *  selects LRC (b1=0) or CRC (b1=1).
 */
static uint8_t atr_scan_protocols(const struct card_handle *ch, uint32_t *mask,
				  enum t1_edc *edc)
{
	unsigned int i = 1;		/* index of T0; TS has been skipped */
	uint8_t y, first = 0;
	bool have_first = false;
	bool want_t1_tc = false;
	...
	while (1) {
		...
		if (y & 0x4) {			/* TCi */
			if (i >= ch->atr.len)
				break;
			if (want_t1_tc) {
				*edc = (ch->atr.atr[i] & 0x01) ? T1_EDC_CRC : T1_EDC_LRC;
				want_t1_tc = false;
			}
			i++;
		}
		if (!(y & 0x8))			/* no TDi, so no further rounds */
			break;
		if (i >= ch->atr.len)
			break;

		*mask |= 1 << (ch->atr.atr[i] & 0x0f);
		if (!have_first) {
			first = ch->atr.atr[i] & 0x0f;
			have_first = true;
		}
		if ((ch->atr.atr[i] & 0x0f) == 1)
			want_t1_tc = true;
...
out:
	...
	return have_first ? first : 0;
}

/* the state the data phase runs in for the protocol currently in force */
static enum iso7816_3_card_state data_phase_state(const struct card_handle *ch)
{
	return (ch->proto == 1) ? ISO_S_IN_T1 : ISO_S_WAIT_TPDU;
}

/* the ATR, or a PPS exchange, settled the protocol: enter the matching phase */
static void enter_data_phase(struct card_handle *ch)
{
	if (ch->proto == 1) {
		t1_init(&ch->t1, ch->t1_edc, T1_DEFAULT_IFS);
		card_set_state(ch, ISO_S_IN_T1);
	} else {
...
static int tx_byte_atr(struct card_handle *ch)
{
	...
	if (ch->atr.idx < ch->atr.len) {
		...
		emu_update_wt(ch);
		/* work out which protocol the ATR just offered; the first one
		 * named applies unless the reader runs a PPS exchange */
		ch->proto = atr_scan_protocols(ch, &ch->atr_proto_mask, &ch->t1_edc);
		if (!((1 << ch->proto) & SUPPORTED_PROTOCOLS)) {
			TRACE_ERROR("%u: ATR offers T=%u first, which is not "
				    "implemented; falling back to T=0\r\n",
				    ch->num, ch->proto);
			ch->proto = 0;
		}
		TRACE_INFO("%u: ATR offers protocols 0x%x, using T=%u\r\n",
			   ch->num, (unsigned int) ch->atr_proto_mask, ch->proto);
		/* go to next state */
		enter_data_phase(ch);
		return 0;
	}
	...
	case PTS_S_WAIT_REQ_PCK:
		...
		if (ch->pts.req[_PCK] != csum_pts(ch->pts.req)) {
			TRACE_ERROR("%u: Error in PTS Checksum!\r\n",
				    ch->num);
			/* stay on the protocol we were already using */
			set_pts_state(ch, PTS_S_WAIT_REQ_PTSS);
			return data_phase_state(ch);
		}
		/* Only acknowledge a protocol we can actually speak and that
		 * this ATR offered. Echoing the request unconditionally meant
		 * agreeing to e.g. T=1 and then continuing to send T=0, which
		 * silently breaks the link. ISO 7816-3 section 9.3: a card that
		 * does not support the proposal stays mute, upon which the
		 * reader resets and falls back to the ATR's first protocol. */
		proto = ch->pts.req[_PTS0] & 0x0f;
		if (!((1 << proto) & SUPPORTED_PROTOCOLS & ch->atr_proto_mask)) {
			TRACE_INFO("%u: PTS proposes T=%u which we did not offer "
				   "(ATR mask 0x%x), staying mute\r\n", ch->num,
				   proto, (unsigned int) ch->atr_proto_mask);
			set_pts_state(ch, PTS_S_WAIT_REQ_PTSS);
			return data_phase_state(ch);
		}
...
static int tx_byte_pts(struct card_handle *ch)
{
	...
	case PTS_S_WAIT_RESP_PCK:
		...
		emu_update_wt(ch);
		/* the exchange completed, so the proposed protocol is now the
		 * one in force */
		ch->proto = ch->pts.resp[_PTS0] & 0x0f;
		TRACE_INFO("%u: PTS agreed on T=%u\r\n", ch->num, ch->proto);
		enter_data_phase(ch);
		set_pts_state(ch, PTS_S_WAIT_REQ_PTSS);
		break;
	default:
...
/* hand the INF of one received I-block to the host */
static void t1_submit_rx(struct card_handle *ch, bool final)
{
	struct msgb *msg;
	struct cardemu_usb_msg_rx_data *rd;
	uint8_t *cur;
	...
	msg = usb_buf_alloc_st(ch->in_ep, SIMTRACE_MSGC_CARDEM,
			       SIMTRACE_MSGT_DO_CEMU_RX_DATA);
	if (!msg) {
		TRACE_ERROR("%u: T=1 RX but ENOMEM\r\n", ch->num);
		return;
	}

	rd = (struct cardemu_usb_msg_rx_data *) msgb_put(msg, sizeof(*rd));
	rd->flags = CEMU_DATA_F_T1_BLOCK;
	if (final)
		rd->flags |= CEMU_DATA_F_FINAL;
	rd->data_len = ch->t1.rx_inf_avail;
	if (ch->t1.rx_inf_avail) {
		cur = msgb_put(msg, ch->t1.rx_inf_avail);
		memcpy(cur, ch->t1.rx_inf, ch->t1.rx_inf_avail);
	}

	usb_buf_upd_len_and_submit(msg);
}

/* turn the next response chunk the host queued into an I-block */
static void t1_feed_from_host(struct card_handle *ch)
{
	struct cardemu_usb_msg_tx_data *td;
	struct msgb *msg;
	unsigned long x;
	...
	msg->l1h = msg->head;
	msg->l2h = msg->l1h + sizeof(struct simtrace_msg_hdr);
	td = (struct cardemu_usb_msg_tx_data *) msg->l2h;

	/* the host marks the last chunk; anything before it chains */
	more = !(td->flags & CEMU_DATA_F_FINAL);

	if (td->data_len > T1_MAX_IFS) {
		TRACE_ERROR("%u: T=1 response chunk of %u bytes exceeds the "
			    "%u byte maximum\r\n", ch->num, td->data_len, T1_MAX_IFS);
	} else if (t1_tx_inf(&ch->t1, td->data, td->data_len, more) < 0) {
		TRACE_ERROR("%u: T=1 response chunk of %u bytes exceeds IFSD %u\r\n",
			    ch->num, td->data_len, ch->t1.ifsd);
	}

	/* This chunk answers the reader. Any further chunk the host has already
	 * queued belongs to the next R-block, not to this turn. */
	ch->t1_owe_reader = false;

	usb_buf_free(msg);
}

/* act on what the block layer reported about a received byte */
static void t1_handle_events(struct card_handle *ch, uint32_t ev)
{
	if (ev & T1_EV_RX_DATA) {
		bool complete = (ev & T1_EV_CMD_COMPLETE) ? true : false;

		t1_submit_rx(ch, complete);
		/* a complete command means the host will answer, and the reader
		 * is now waiting on us */
		if (complete)
			ch->t1_owe_reader = true;
	}

	/* the reader acknowledged our last block, so send the next chunk. It may
	 * not have arrived over USB yet, in which case the flag makes
	 * card_emu_have_new_uart_tx() send it the moment it does. */
	if (ev & T1_EV_NEED_NEXT_CHUNK) {
		ch->t1_owe_reader = true;
		t1_feed_from_host(ch);
	}

	/* whatever the block layer queued goes out now */
	if (t1_tx_pending(&ch->t1))
		card_emu_uart_enable(ch->uart_chan, ENABLE_TX);
}

/* transmit a single byte of the current T=1 block */
static int tx_byte_t1(struct card_handle *ch)
{
	uint8_t byte;

	/* Nothing queued? Take the host's response, but only when the reader is
	 * actually waiting on us -- a chunk for the next R-block must stay in
	 * the queue until that R-block arrives. */
	if (!t1_tx_pending(&ch->t1) && ch->t1_owe_reader)
		t1_feed_from_host(ch);

	if (!t1_get_tx_byte(&ch->t1, &byte)) {
		/* the block is fully out, so go back to listening */
		card_emu_uart_enable(ch->uart_chan, ENABLE_RX);
		return 0;
	}
...
void card_emu_process_rx_byte(struct card_handle *ch, uint8_t byte)
{
	...
	case ISO_S_IN_TPDU:
		...
		break;
	case ISO_S_IN_T1:
		/* A PPS may still arrive before the first block. FF is not a
		 * valid NAD, so between blocks it can only be a PPSS. */
		if (byte == 0xff && ch->t1.rx_state == T1_RX_NAD) {
			set_pts_state(ch, PTS_S_WAIT_REQ_PTSS);
			new_state = process_byte_pts(ch, byte);
			ch->stats.pps++;
			goto out_silent;
		}
		t1_handle_events(ch, t1_rx_byte(&ch->t1, byte));
		goto out_silent;
	case ISO_S_IN_PTS:
		new_state = process_byte_pts(ch, byte);
		goto out_silent;
...
void card_emu_have_new_uart_tx(struct card_handle *ch)
{
	...
	case ISO_S_IN_TPDU:
		...
		break;
	case ISO_S_IN_T1:
		/* The host produced a response chunk, but a chunk only goes out
		 * when it is our turn to speak. A response longer than IFSD
		 * arrives as several messages in quick succession, and enabling
		 * the transmitter for each of them would reconfigure the USART
		 * on top of the block still going out, then put the next chunk
		 * on the line without waiting for the reader's R-block -- into
		 * the reader's own transmission, the line being half-duplex.
		 * Chunks after the first are pulled by T1_EV_NEED_NEXT_CHUNK
		 * instead. */
