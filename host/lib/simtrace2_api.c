...
/*! \brief when set, every message to and from the device is hexdumped.
 *  Off by default: the once-a-second status poll would otherwise bury
 *  everything else in the log. */
int osmo_st2_verbose = 0;
...
/*! \brief Request the SIMtrace2 to send one chunk of a T=1 response.
 *  Each chunk becomes exactly one I-block; \a final clears the chaining bit. */
int osmo_st2_cardem_request_t1_tx(struct osmo_st2_cardem_inst *ci, const uint8_t *data,
				  uint16_t len, bool final)
{
	...
	printf("<= %s(%s, final=%d)\n", __func__, osmo_hexdump(data, len), final);

	memset(txd, 0, sizeof(*txd));
	txd->data_len = len;
	txd->flags = CEMU_DATA_F_T1_BLOCK;
	if (final)
		txd->flags |= CEMU_DATA_F_FINAL;
...
/*! \brief Ask the SIMtrace2 for the current card interface status.
 *  The firmware answers on the bulk IN endpoint with a cardemu_usb_msg_status,
 *  which is the only way to learn the present VCC/CLK/RST state: the
 *  CEMU_FEAT_F_STATUS_IRQ reports are edge-triggered and say nothing about a
 *  state that was already settled before we connected. */
int osmo_st2_cardem_request_status(struct osmo_st2_cardem_inst *ci)
{
	struct msgb *msg = st_msgb_alloc();

	return osmo_st2_slot_tx_msg(ci->slot, msg, SIMTRACE_MSGC_CARDEM, SIMTRACE_MSGT_BD_CEMU_STATUS);
}
